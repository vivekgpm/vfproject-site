...
const ManageAssetTransaction = () => {
  ...
  useEffect(() => {
    const fetchAssetPurchases = async () => {
      try {
        ...
        const assetPurchasesQuery = query(assetPurchasesRef, orderBy('createdAt', 'desc'));
  ...
  const filteredAssets = assetPurchases.filter(asset => 
          ...
          <tbody>
            {paginatedAssets.map((asset) => (
              <tr key={asset.id}>
                ...
                <td>₹{asset.pricing?.totalPrice?.toLocaleString("en-IN") || '0'}</td>
                <td>₹{asset.bookingAmount?.toLocaleString("en-IN") || '0'}</td>
                <td>₹{asset.pricing?.remainingPayment?.toLocaleString("en-IN") || '0'}</td>
                <td>
